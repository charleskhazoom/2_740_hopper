...
syms tau1 tau2 tau3 Fx Fy F_boom real
syms Ir N real
syms m_offset_x m_offset_y l_boom h_boom hob k real
...
p   = [m1 m2 m3 m4 m_body m_arm I1 I2 I3 I4 I_arm Ir N l_O_m1 l_B_m2...
    ...
    m_offset_x m_offset_y l_boom h_boom hob k]';        % parameters
...
Vg_body = (m_body+m_offset_y)*g*dot(r_body_cm, -ihat);

T = simplify(T1 + T2 + T3 + T4 + T_body + T_arm + T1r + T2r + T3r);
Vg = Vg1 + Vg2 + Vg3 + Vg4 + Vg_arm + Vg_body;

% manually add boom force to eom
th_boom = asin((h_boom - (y+hob))/l_boom);
F_boom = simplify(2*k*(th_boom+pi)/(l_boom*cos(th_boom)));
...
Q_F_E = F2Q(F,rE(1:2));
Q_F_boom = F2Q(F_boom,r_origin(2));
Q_F = Q_F_E + Q_F_boom;
...
if write_dynamics_files==true
    ...
    matlabFunction(dr_arm_tip,'file',[path_sym_dynamics,'arm_tip_vel'],'vars',{z p});
end
