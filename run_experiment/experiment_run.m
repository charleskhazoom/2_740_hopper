...
data = load('final_traj_with_arm.mat'); % load in trajectory data
...
hip_flight = q_profile(1,end);    % initial hip
knee_flight = q_profile(2,end);  % initial knee
arm_flight = q_profile(3,end);   % inital arm
...
angle3_init = pi;            % arm angle  % 142 deg (at extremes, min max)
...
pre_buffer_time   = 4;      % give hopper 3s to reach initial config 
post_buffer_time  = 4;

time_param = [traj_time pre_buffer_time post_buffer_time]';

% Gains for joint impedance controller
gains.K_q1 = 1;             % proportional gain for hip angle
gains.K_q2 = 1;             % proportional gain for knee angle
gains.K_q3 = 1;             % proportional gain for arm angle
...
[output_data] = experiment_dataHandler( time_param, q0, gains, control_method,...
                                       ...
                                       hip_flight, knee_flight, arm_flight);
